...
\todo{Kunne godt tænke mig at vi her beskrev problemet med at vi ikke kan lære features for par direkte, men vi lærer dem for noder, og kombinerer disse. Se udkommentering for eksempel.}
...
In order to do generate these features we use an algortimic framework for semi-supervised feature learning called node2vec~\cite{node2vec}. Node2vec is a way of creating feature sets \todo{vectors?} for nodes in a graph based on its structure, which can then be used to identify characteristics of a relation between two nodes. After identifying a feature representation of these characteristics, the classifier component can use them as input features for candidate pairs. We choose to use node2vec~\cite{node2vec} based on reports of its performance as well as its flexibility to adapt to different graph structures.
\todo{Kan node2vec intro bare sige at det er network embedding?}
