...
The database contains roughly 5 million articles squared, which totals to $2.5 \times 10^{13}$ article pairs. Suppose we can process 1 million of these pairs per second, it will still take almost 290 days to process all pairs. \todo{since we do not wish to work this slow, or have the require a certain amount of computer power this problem of input size needs to be solve.} However, since the majority of these article pairs will not need a link, we can avoid generating all possible pairs, and instead select article pairs based on a heuristic policy that orders the pairs. A perfect ordering would be where all the articles with missing links occur before any article without missing links. While such a policy would be helpful, it is unrealistic and only serves as a benchmark. As the policy is a heuristic, it only needs to perform reasonably well, which is why we do not employ any systematic evaluation of possible policies, but our choice is solely based on our own intuition. As long as a sufficient number of pairs are generated to not bottleneck the main pipeline, this way of choosing a policy will be acceptable.
