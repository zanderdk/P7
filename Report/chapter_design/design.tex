...
The feature extractor receives a pair of articles and extracts features from a node2vec model that has been trained ahead of time. The feature pair are represented as two vectors which is combined using a operation such as multiplication \todo{maybe write that we always multiply them.} and used as input for the AI.
