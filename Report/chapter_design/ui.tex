...
\cref{fig:information_flow_UI} shows two actors and the UI\@\todo{uklart. Simon: hvordan uklart?}. One actor is the \emph{Wikipedia Editor} and the other is the \emph{Main Pipeline}. While the main pipeline could be modeled as a system with its own information flow, it is more beneficial for the requirement engineering process to simplify the main pipeline as an actor outside of the UI, since the requirements for the UI should not be affected by the inner workings of the main pipeline\todo{lang sætning, bør måske omskrives. Simon: jeg kan ikke finde ud af hvor man splitter :/}.
...
The last step of forwarding the review to the main pipeline represents a future use of the reviews being used as feedback for the classifier, which can improve predictions. This is not a current consideration in the main pipeline, but the UI is designed to support this type of feedback\todo{Ved ikke lige hvor vigtigt det er at beskrive det her? Simon: jeg synes det er fint, viser vi tænker arkitekturen i gennem}. The requirements found in these two information flows are:
