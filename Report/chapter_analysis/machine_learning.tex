...
To give a precise understanding of the binary classification problem, we will define it formally before explaining how we intend to solve it.
...
As classifiers generally operates on feature vectors, we first need to represent article pairs as appropriate feature vectors. To do this we define $f: V\times V \to \mathbb{R}^d$ as a function, mapping article pairs to feature vectors, where $d$ is the dimension of the feature vector.
...
\end{equation}
 ...
 We can then used the function $h$ to assign a label $y_{ab}$ to any article pair $(a,b)$ using \cref{eq:classification_function}.
