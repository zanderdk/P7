...
Because the feature learning process is mostly autonomous, the parameter space of the feature learning algorithm can be systematically explored, in order to improve the feature representation without understanding the features generated.
\todo{Kasper omskriv: Det er smart at det hele kører automatisk.}

The approach of using feature learning on graphs can be seen as network embedding.\todo{magi + kilde + forklar network embedding kort} So in our case, instead of manually defining the function $f$, we seek to learn it based on the graph representation of Wikipedia. We will further elaborate on network embedding and describe a concrete feature learning algorithm in \cref{feature_extractor}. \todo{Tilpas efter todo er ordnet.}
