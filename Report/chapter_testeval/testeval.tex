...
% vi træner kun på featured. positiver og negativer. Disse artikler er meget forskellige fra alle andre artikler
% vi har brug for flere varierede eksempler
% vi fik ikke cleanet dataen sådan så ikke-eksisterende artikler er med. 
% vi har ikke nok positiver
% vores test set er dårligt, da det ikke afspejler artikler på hele wiki
% Vores negativer er ikke representable for realistiske test eksempler
% Node2vec modellen er optimiret til at 

%A reason that the training data does not generalize to the whole of Wikipedia is probably due to the low amount of training data. :(
...
\todo{review/discuss this.}

%By removing them we could have gained a better representation of the actual link structure on Wikipedia.
